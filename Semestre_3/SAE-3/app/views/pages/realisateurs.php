...
require_once $rootPath . '/app/models/Realisateur.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_realisateur') {
    $deleteController = new DeleteController($pdo);
    $deleteController->deleteRealisateur();
}
...
try {
    $realisateurModel = new Realisateur($pdo);
    
    // Utilisation de la méthode avec filtres
    $realisateurs = $realisateurModel->getRealisateursWithFilters($search, $categorie_id);
    $categories = $realisateurModel->getAllCategories();
    
} catch (PDOException $e) {
    $error = "Erreur: " . $e->getMessage();
    error_log("Erreur realisateurs.php: " . $e->getMessage());
}
...
<link rel="stylesheet" href="/sae-3-festivote-tas-cesar/public/css/realisateurs.css">
<link rel="stylesheet" href="/sae-3-festivote-tas-cesar/public/css/vote-modal.css">

<div class="categories-container">
    <div class="categories-header">
        <h1><img src="<?= ImageHelper::url('img/logo/logo-cam.png') ?>" alt="logo-real" width="60" class="img-container">  Les Réalisateurs</h1>
        ...
        <?php if (isset($_SESSION['role']) && ($_SESSION['role'] === 'admin' || $_SESSION['role'] === 'candidat')):
            ...
            <a href="/sae-3-festivote-tas-cesar/app/views/admin/add-realisateur.php" class="btn-add">➕ Ajouter un realisateur</a>
            <?php endif; ?>
        <?php endif; ?>
    </div>
    ...
    <div class="filters-section">
        <form method="GET" action="" class="filters-form">
            <div class="filter-group">
                <label for="search">Rechercher un réalisateur</label>
                <input 
                    type="text" 
                    id="search" 
                    name="search" 
    ...
    <?php if (!empty($search) || !empty($categorie_id)): ?>
        <div class="filter-info">
            <p>
                <?= count($realisateurs) ?> résultat(s) trouvé(s)
                <?php if ($search): ?>
            ...
            <?php foreach ($realisateurs as $real): ?>
                ...
                <div class="film-card">
                    <a href="/sae-3-festivote-tas-cesar/app/views/public/detail-realisateur.php?id=<?= $real['id_realisateur'] ?>">
                        ...
                        <div class="film-info">
                            <h3><?= htmlspecialchars($real['prenom'] . ' ' . $real['nom']) ?></h3>
                            <p class="film-year"><?= htmlspecialchars($real['nationalite'] ?? 'Nationalité inconnue') ?></p>
                            <p class="film-genres"><img src="<?= ImageHelper::url('img/logo/logo-bobine.png') ?>" alt="logo-film" width="40" class="img-container"> <?= intval($real['nb_films']) ?> film(s)</p>
                        </div>
                    </a>
                    ...
                    <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'electeur' && $isInPeriodeVote): ?>
                        <button class="btn-vote"
                            ...
                            data-id="<?= $real['id_realisateur'] ?>"
                            data-name="<?= htmlspecialchars($real['prenom'] . ' ' . $real['nom']) ?>">
                            Voter
                        </button>
                    <?php endif; ?>
                    <?php if ($isAdmin): ?>
                        <button type="button" class="btn-cancel" onclick="openDeleteModal('realisateur', <?= $real['id_realisateur'] ?>, '<?= htmlspecialchars(addslashes($real['prenom'] . ' ' . $real['nom'])) ?>')">
                            Supprimer
                        </button>
                    <?php endif; ?>
                </div>
...
<div id="deleteModal" class="modal">
    <div class="modal-content">
        ...
        <form id="deleteForm" method="POST">
            <?= CsrfHelper::generateField() ?>
            <input type="hidden" name="action" value="delete_realisateur">
